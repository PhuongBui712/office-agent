...
# GOOD: Parameterized query (psycopg/asyncpg/SQLAlchemy text())
...
```python
# BAD: Returning raw HTML built from user input (Jinja autoescape disabled / |safe)
return Markup(f"<div>{user_comment}</div>")

# GOOD: Rely on autoescape; never apply |safe to untrusted content
return render_template("comment.html", body=user_comment)
```
...
```python
# BAD: Deep nesting + in-place mutation
...
# GOOD: Early return + comprehension + dataclass replace (immutability)
from dataclasses import replace
def process_users(users):
    if not users:
        return []
    return [replace(u, verified=True) for u in users if u.active and u.email]
```

### React/Next.js Patterns (HIGH)

*Skip this section if the project is not React/Next.js.* When reviewing React/Next.js code, also check:
...
```python
# BAD: N+1 with SQLAlchemy ORM
users = session.query(User).all()
for u in users:
    u.posts          # triggers a SELECT per user
...
### Backend / Python Patterns (HIGH)

*Skip if the project is not a Python service.* When reviewing FastAPI / Flask / Django / async Python:

- **Sync I/O inside `async def`** — `requests.get(...)`, `time.sleep`, blocking DB drivers in async handlers. Use `httpx.AsyncClient`, `asyncio.sleep`, async drivers (asyncpg, aiomysql).
- **Mutable default arguments** — `def f(items=[])` reuses the same list across calls. Use `None` and assign inside the body.
- **Missing Pydantic / dataclass validation at request boundary** — accepting `dict[str, Any]` from a JSON body and forwarding to business logic.
- **Misused FastAPI `Depends`** — running heavy work in a dependency without caching, or instantiating clients on every request instead of via `lifespan` / module-level singletons.
- **`except Exception` without re-raise or log context** — see also `silent-failure-hunter`.
- **Logging f-strings with PII** — `logger.info(f"user={user.email}")` leaks PII into logs and bypasses structured logging.
- **Missing `with`/context managers** — file handles, DB sessions, locks must be wrapped or released in `finally`.
- **Missing timeouts on outbound calls** — `httpx.get(url)` without `timeout=` blocks the event loop indefinitely.
